...
func character_setup() -> void:
	...
	await get_tree().physics_frame  # Wait one more frame to be extra sure
...
func _on_physics_process(_delta : float) -> void:
	...
	# if navigation agent 2d node has Avoidance enabled 
	if navigation_agent_2d.avoidance_enabled:
		animated_sprite_2d.flip_h = velocity.x > 0
		navigation_agent_2d.velocity = velocity # set velocity to NavigationAgent2D, calls on_safe_velocity_comuputed
	# if avoidance if disabled asign velocity as normal 
	else: 
